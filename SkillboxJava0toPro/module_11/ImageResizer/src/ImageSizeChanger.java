...
public class ImageSizeChanger implements Callable<Integer> {
  ...
  public Integer call() {
    try {
      for (File file : files) {
        ...
        BufferedImage newImage = getResizedBufferedImage(image);
  ...
  public BufferedImage getResizedBufferedImage(BufferedImage image) {
    int tmpImgWidth = newWidth * 2;
    int tmpImgHeight = (int) Math.round(image.getHeight() / (image.getWidth() / (double) tmpImgWidth));
